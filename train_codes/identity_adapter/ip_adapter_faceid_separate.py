...
from .utils import register_cross_attention_hook, get_net_attn_map, attnmaps2images
...
class FacePerceiverResampler(torch.nn.Module):
    def __init__(
        ...
        *,
        ...
        self.proj_in = torch.nn.Linear(embedding_dim, dim * num_tokens)
...
class ProjPlusModel(torch.nn.Module):
    def __init__(self, cross_attention_dim=768, id_embeddings_dim=512, clip_embeddings_dim=1280, num_tokens=4):
        ...
        print(f"cross_attention_dim: {cross_attention_dim},id_embeddings_dim: {id_embeddings_dim}, clip_embeddings_dim:{clip_embeddings_dim}, num_tokens:{num_tokens}")
    ...
    def forward(self, id_embeds, clip_embeds, shortcut=False, scale=1.0):
        print(f"id_embeds.shpe: {id_embeds.shape}")
        x = self.proj(id_embeds)
        x = x.reshape(-1, self.num_tokens, self.cross_attention_dim)
        x = self.norm(x)
        clip_embeds = clip_embeds.squeeze(0).view(8, -1)
        print(f"x.shape: {x.shape}, clip_embeds.shape: {clip_embeds.shape}")
...
class IPAdapterFaceID(torch.nn.Module):
    def __init__(self,unet, ip_ckpt, device, num_tokens=4, n_cond=1, torch_dtype=torch.float32, use_controlnet = False, use_linear_proj=True, controlnet_model_name_or_path = '', cond_scale=1.0):
        super().__init__()
        self.device = device
        ...
        # self.image_proj_model = self.init_plus_proj()
        # self.load_ip_adapter()

    def init_plus_proj(self):
    ...
    def init_controlnet(self):
        ...
        if self.controlnet_model_name_or_path:
    ...
    def set_ip_adapter(self):
        ...
        self.unet = register_cross_attention_hook(self.unet)

    def load_ip_adapter(self):
        state_dict = {"image_proj": {}, "identity_adapter": {}}
        if os.path.splitext(self.ip_ckpt)[-1] == ".safetensors":
            print(f"using safe tensor: {self.ip_ckpt}")
            with safe_open(self.ip_ckpt, framework="pt", device="cpu") as f:
                # print(f"keys in safetensors: {f.keys()}")
                for key in f.keys():
                    if key.startswith("image_proj_model."):
                        state_dict["image_proj_model"][key.replace("image_proj_model.", "")] = f.get_tensor(key)
                    elif key.startswith("identity_adapter."):
                        state_dict["adapter_modules"][key.replace("adapter_modules.", "")] = f.get_tensor(key)
        else:
            print(f"using torch.load: {self.ip_ckpt}")
            ckpt_dict = torch.load(self.ip_ckpt, map_location=self.device)
            print(f"ckpt_dict.keys(): {ckpt_dict.keys()}")
            for key in ckpt_dict.keys():
                if key.startswith("image_proj_model."):
                    state_dict["image_proj"][key.replace("image_proj_model.", "")] = ckpt_dict[key]
                elif key.startswith("adapter_modules."):
                    state_dict["identity_adapter"][key.replace("identity_adapter.", "")] = ckpt_dict[key]
                elif key.startswith('image_proj'):
                    state_dict["image_proj"] = ckpt_dict[key]
                elif key.startswith('identity_adapter'):
                    state_dict["identity_adapter"] = ckpt_dict[key]
        print(f"state_dict: {state_dict.keys()}")
        self.image_proj_model.load_state_dict(state_dict["image_proj"])
        ip_layers = torch.nn.ModuleList(self.unet.attn_processors.values())
        ip_layers.load_state_dict(state_dict["identity_adapter"], strict=False)

    # @torch.inference_mode()
    # def get_image_embeds_plus(self, faceid_embeds, face_latents, s_scale, shortcut):
    #     multi_face = False
    #     print(f"faceid_embeds.dim(): {faceid_embeds.dim()}")
    #     if faceid_embeds.dim() == 3:
    #         multi_face = True
    #         b, n, c = faceid_embeds.shape
    #         faceid_embeds = faceid_embeds.reshape(b*n, c)
    #     faceid_embeds = faceid_embeds.to(self.device, dtype=self.torch_dtype)
    #     image_prompt_embeds = self.image_proj_model(faceid_embeds, face_latents, shortcut=shortcut, scale=s_scale)
    #     if multi_face:
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, image_embeds, kps_img=None):
        # print(f"image_embeds.shape:{image_embeds.shape}")
        image_prompt_embeds = self.get_image_embeds(image_embeds)
        # print(f"noisy_latents.shape:{noisy_latents.shape},\n image_prompt_embeds.shape: {image_prompt_embeds.shape},\n encoder_hidden_states.shape:{encoder_hidden_states.shape}")
        ...
        if kps_img is None:
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                ...
                controlnet_cond=kps_img,
                conditioning_scale=self.cond_scale,
            )
            # Predict the noise residual
            noise_pred = self.unet(noisy_latents, timesteps, encoder_hidden_states, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample).sample
        return noise_pred
            ...
            with safe_open(self.ip_ckpt, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("image_proj."):
                        ...
                        state_dict["identity_adapter"][key.replace("identity_adapter.", "")] = f.get_tensor(key)
        else:
