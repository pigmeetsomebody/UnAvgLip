...
class IPAdapterModule(torch.nn.Module):
    ...
    """IP-Adapter"""
    def __init__(self, unet, image_proj_model, adapter_modules, device, torch_dtype, ckpt_path=None):
        super().__init__()
        self.unet = unet
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, image_embeds):
        if isinstance(self.image_proj_model, MLPProjModel):
            ip_tokens = self.get_image_embeds(image_embeds)
        else:
            print(f"faceid_embeds.dim(): {image_embeds.dim()}")
            ip_tokens = self.image_proj_model(image_embeds)
        print(f"ip_tokens.shape: {ip_tokens.shape}")
    ...
    def load_from_checkpoint(self, ckpt_path: str):
        ...
        print(f"state_dict: {ckpt_dict.keys()}")
        ...
        for key in ckpt_dict.keys():
            if key.startswith("image_proj_model."):
                ...
                state_dict["ip_adapter"][key.replace("adapter_modules.", "")] = ckpt_dict[key]
