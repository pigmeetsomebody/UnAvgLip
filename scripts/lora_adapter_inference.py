...
from models.GPEN.gpen_face_enhancer import FaceEnhancement
...
def selected_random_face_embedings(frames, target_num, save_dir, vidname):
    ...
    while i < target_num:
        ...
        # y1, y2 = height // 2, height
        cropped_lip_image = Image.fromarray(cv2.cvtColor(random_frame, cv2.COLOR_BGR2RGB))
        ...
        if len(faces) <= 0:
            ...
            print(f"select face image as embedding: {vidname} in {image_path}")
...
enhancer = FaceEnhancement(base_dir='/home/zhuyy/video-retalking/checkpoints/', size=512, model='GPEN-BFR-512', use_sr=False, \
...
def main(args):
    ...
    for task_id in inference_config:
        ...
        output_basename = f"{input_basename}_{audio_basename}_lora_adapter_{args.guide_scale}_selected"
        ...
        enhance_image_save_dir = os.path.join(result_img_save_path, f"{output_basename}_enhanced")
        os.makedirs(enhance_image_save_dir,exist_ok =True)
        ...
        crop_frames_enhanced = []
        for idx in tqdm(range(len(crop_frames)), desc='[Step 5] Reference Enhancement'):
            crop_frame = crop_frames[idx]
            pred, _, _ = enhancer.process(crop_frame, crop_frame, face_enhance=True, possion_blending=False)
            crop_frames_enhanced.append(pred)
            save_path = os.path.join(enhance_image_save_dir, f"{idx}.png")
            cv2.imwrite(save_path, pred)

        crop_frames = crop_frames_enhanced
        ...
        for i, res_pil in enumerate(tqdm(res_frame_list)):
            ...
            if (args.crop_path == None) or not os.path.exists(args.crop_path):
                ...
                cv2.imwrite(f"{face3dmm_save_dir}/{str(i).zfill(8)}.png", res_frame)
                x1, y1, x2, y2 = bbox 
                try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_ip_adapter_path", type=str, default="./train_codes/output/adapter0116/checkpoint-2000")
