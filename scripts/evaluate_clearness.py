...
def main(args):
    ...
    for task_id in evaluate_config:
        ...
        # handle generated_video_path
        if get_file_type(generated_video_path)=="video":
            generated_input_basename = os.path.basename(generated_video_path).split('.')[-2]
            save_generated_dir = os.path.join(result_save_path, "generated")
            ...
            raise ValueError(f"{generated_video_path} should be a video file, an image file or a directory of images")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sync_net_path', type=str, default=None)
